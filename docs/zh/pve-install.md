...
想要一个能正常显示中文的“控制台”，给虚拟机加一个串口即可（渲染同样发生在浏览器端）：

- 网页界面：虚拟机 → 硬件 → 添加 → 串行端口
- 或在 PVE 宿主机 shell 执行：`qm set <vmid> --serial0 socket`

之后控制台下拉菜单会出现 **xterm.js** 终端，选择它即可正常显示中文。

注：串口**无法**通过 OVA 内置 —— PVE 的 OVF 导入器不会为串口设备创建硬件，部分导入器（含某些 VMware 路径）遇到 OVF 里的串口条目甚至会直接报错；所以串口需要在导入后手动添加一次（如上）。
