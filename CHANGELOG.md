...
- Generate `en_US.UTF-8` alongside the configured locale: Landscape's web terminal spawns its shell with `LC_ALL=en_US.UTF-8`, so every session on the locale-trimmed image opened with `bash: warning: setlocale: LC_ALL: cannot change locale (en_US.UTF-8)`; the PVE guide also gains an FAQ explaining that CJK shows as boxes in the noVNC console because the kernel console font is capped at ~512 glyphs (inherent limitation — use SSH or the web terminal for Chinese)\
  在配置的 locale 之外固定生成 `en_US.UTF-8`：Landscape 的 Web 终端以 `LC_ALL=en_US.UTF-8` 启动 shell，此前在裁剪过 locale 的镜像上每个会话都会先报 `bash: warning: setlocale: LC_ALL: cannot change locale (en_US.UTF-8)`；PVE 指南新增 FAQ，说明 noVNC 控制台中文显示为方块源于内核控制台字体约 512 字形的上限（固有限制，中文交互请用 SSH 或 Web 终端）
