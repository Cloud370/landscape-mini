...
- Custom Build download UX: the workflow summary now ends with a fenced "copy-ready latest links" block (one full URL per published asset, `.ova` included), the tutorials document the deterministic asset naming rule (`landscape-mini-x86-<base>[-docker][-lkit]`) so the `custom-build-latest` direct URL is predictable and can be hard-coded into PVE's URL import, and the PVE guide's storage navigation now follows the real sidebar flow (sidebar storage entry → right-panel Edit to enable content types → Content sub-page for the import)\
  Custom Build 下载体验优化：workflow 摘要末尾新增「复制即用的 Latest 直链」代码块（每个已发布产物一行完整 URL，含 `.ova`）；教程写明产物命名的确定性规则（`landscape-mini-x86-<base>[-docker][-lkit]`），`custom-build-latest` 直链因此可预测、可直接写死进 PVE 的 URL 导入框；PVE 指南的存储导航改为真实的侧边栏路径（侧边栏存储条目 → 右侧面板 Edit 开启内容类型 → Content 子页执行导入）
- `INCLUDE_LKIT` now defaults to `true`: Debian images embed the lkit-managed layout out of the box, the global `lkit` command (`/usr/local/bin/lkit`) is on every user's `PATH`, and the SSH login banner prints a gated landscape-kit guidance section (`lkit check` / `lkit update` / `lkit --help`) whenever lkit is embedded; local non-Debian builds warn and fall back to the legacy layout instead of failing, the reusable build workflow's default flips with it, and the Custom Build workflow defaults `include_lkit` to true with an explicit error guiding Alpine users to switch it off\
  `INCLUDE_LKIT` 默认改为 `true`：Debian 镜像开箱即用 lkit 托管布局，全局 `lkit` 命令（`/usr/local/bin/lkit`）对所有用户的 `PATH` 可用，SSH 登录欢迎信息在检测到 lkit 时显示引导段（`lkit check` / `lkit update` / `lkit --help`）；本地非 Debian 构建警告并回退 legacy 布局而不再报错退出，复用构建工作流默认值同步翻转，Custom Build 的 `include_lkit` 默认开启并对 Alpine 组合给出明确报错引导
...
- Fix `expand-rootfs.service` failing on first boot of rootless-built images: the gid lookup passed `awk -v g "$group"` — two arguments instead of the required `var=value` form — so mawk aborted with `improper assignment` (exit 2, stderr swallowed by `2>/dev/null`) and `set -e` killed the script inside the first `restore_group` call, leaving the service `FAILED` at boot and the setgid group restore never applied (`unix_chkpwd` stayed `root:root`, breaking non-root password logins); the argument is now correctly joined as `-v g="$group"`, and the Debian readiness test asserts both the service state and the restored `unix_chkpwd:shadow` ownership so this class of failure cannot ship green again\
  修复 rootless 构建镜像首次启动时 `expand-rootfs.service` 失败的问题：gid 查找把 `awk -v g "$group"` 写成了两个独立参数而非要求的 `var=value` 形式，mawk 以 `improper assignment` 中止（退出码 2，报错被 `2>/dev/null` 吞掉），`set -e` 使脚本在第一个 `restore_group` 调用处即退出 —— 服务启动显示 `FAILED`，setgid 组恢复从未执行（`unix_chkpwd` 一直是 `root:root`，非 root 密码登录因此失效）；参数现已正确写成 `-v g="$group"`，Debian readiness 测试同时断言服务状态与 `unix_chkpwd:shadow` 属主恢复结果，此类故障无法再带着绿灯发布
- Generate `en_US.UTF-8` alongside the configured locale: Landscape's web terminal spawns its shell with `LC_ALL=en_US.UTF-8`, so every session on the locale-trimmed image opened with `bash: warning: setlocale: LC_ALL: cannot change locale (en_US.UTF-8)`; the PVE guide also gains an FAQ explaining that CJK shows as boxes in the noVNC console because the kernel console font is capped at ~512 glyphs (inherent limitation — use SSH or the web terminal for Chinese)\
  在配置的 locale 之外固定生成 `en_US.UTF-8`：Landscape 的 Web 终端以 `LC_ALL=en_US.UTF-8` 启动 shell，此前在裁剪过 locale 的镜像上每个会话都会先报 `bash: warning: setlocale: LC_ALL: cannot change locale (en_US.UTF-8)`；PVE 指南新增 FAQ，说明 noVNC 控制台中文显示为方块源于内核控制台字体约 512 字形的上限（固有限制，中文交互请用 SSH、Web 终端，或为虚拟机加串口后使用 xterm.js 控制台——串口无法内置进 OVA，PVE 导入器不识别且部分导入器会报错）
